...
import { Users, GraduationCap, AlertCircle, CheckCircle, XCircle, Search, Filter, UserPlus, MessageCircle } from 'lucide-react';

// Mock data to be shared between components
const initialApplications = [
  {
    ...
    goals: ['Improve React skills', 'Learn state management', 'Build a portfolio'],
  ...
  },
  {
    ...
    goals: ['Transition to ML Engineering', 'Learn TensorFlow', 'Complete a data project'],
  ...
  },
  {
    ...
    goals: ['Build a full-stack app', 'Improve database design', 'Learn about APIs'],
  ...
  },
  {
    ...
    goals: ['Improve UX methodologies', 'Learn design systems', 'Create user personas'],
  ...
  },
  {
    ...
    goals: ['Master enterprise Java', 'Build microservices', 'Learn about DevOps'],
...
// App Component to manage shared state between Dashboard and MenteesList
export default function MentorApp() {
  const [applications, setApplications] = useState(initialApplications);
  const [activeView, setActiveView] = useState('dashboard'); // 'dashboard' or 'mentees'
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto p-6">
        {activeView === 'dashboard' ? (
          <MentorDashboard applications={applications} />
        ) : (
          <MenteesList 
...
function MentorDashboard({ applications }) {
  const totalMentees = applications.filter(app => app.status === 'accepted').length;
  const pendingApplications = applications.filter(app => app.status === 'pending').length;
  const pwdMentees = applications.filter(app => app.status === 'accepted' && app.isPWD).length;

  // Only show accepted mentees in the table
  const acceptedMentees = applications.filter(app => app.status === 'accepted');

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold mb-6">Welcome back, Mentor!</h1>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatCard icon={<Users className="w-10 h-10 text-blue-500" />} label="Active Mentees" value={totalMentees} />
        <StatCard icon={<GraduationCap className="w-10 h-10 text-green-500" />} label="Pending Applications" value={pendingApplications} />
        <StatCard icon={<AlertCircle className="w-10 h-10 text-purple-500" />} label="PWD Mentees" value={pwdMentees} />
      </div>
        ...
        <div className="p-6">
          ...
          {acceptedMentees.length === 0 ? (
            <div className="text-center py-8 bg-gray-50 rounded-lg">
              ...
              <h3 className="mt-2 text-sm font-medium text-gray-900">No active mentees</h3>
              <p className="mt-1 text-sm text-gray-500">
                Accept applications to start mentoring!
              </p>
            </div>
          ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-left py-3 px-4">Goals</th>
                ...
                <tbody>
                  {acceptedMentees.map((mentee) => (
                    <tr key={mentee.id} className="border-b hover:bg-gray-50">
                      <td className="py-3 px-4 font-medium">{mentee.menteeName}</td>
                      ...
                      <td className="py-3 px-4">
                        <button
                          onClick={() => alert(`Messaging ${mentee.menteeName}...`)}
                          className="text-blue-600 hover:text-blue-800 mr-3"
                        >
                          Message
                        </button>
                        <button
                          onClick={() => alert(`Viewing training progress for ${mentee.menteeName}...`)}
...
// MenteesList Component
function MenteesList({ applications, acceptApplication, declineApplication }) {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex space-x-3">
          <button
            onClick={() => alert("Finding new mentees...")}
